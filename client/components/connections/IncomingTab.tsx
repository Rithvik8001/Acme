...
import { useIncomingConnections } from "@/hooks/useIncomingConnections";
import { ConnectionCard } from "./ConnectionCard";
import { Button } from "@/components/ui/button";

export function IncomingTab() {
  const { data, nextCursor, loading, loadingMore, error, acting, refetch, loadMore, accept, reject } =
        ...
        {data.map((conn) => (
          <ConnectionCard
            ...
            actions={
              <>
                <button
                  onClick={() => void accept(conn.id)}
                  disabled={acting === conn.id}
                  className="text-[10px] border border-border px-2.5 py-1 text-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary transition-colors disabled:opacity-50"
                >
                  {acting === conn.id ? "…" : "Accept"}
                </button>
                <button
                  onClick={() => void reject(conn.id)}
                  disabled={acting === conn.id}

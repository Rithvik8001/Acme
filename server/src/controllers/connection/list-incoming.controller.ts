...
import listIncomingValidation from "../../validations/connection/list-incoming";
...
const listIncomingController = async (req: Request, res: Response) => {
  const userId = req.userId;
  if (!userId) {
    return sendResult(res, createApiResult(false, "Unauthorized", 401));
  }

  const parsed = listIncomingValidation(req.query);
  if (!parsed.success) {
    return sendResult(
      res,
      createApiResult(
  ...
  try {
    ...
    return sendResult(
      res,
      createApiResult(true, "Incoming requests fetched successfully", 200, {
        data: items,
        nextCursor,
      }),
    );
...
export default listIncomingController;
